...
accountModel.updateProfile = (custId, data, callback) => {
    const sql = `
        ...
        WHERE cust_id = $5
    `;
    const params = [data.first_name, data.last_name, data.phone, data.email, custId];
    db.query(sql, params, (err, results) => {
...
// 3. Thay đổi mật khẩu
accountModel.changePassword = (custId, newPassword, callback) => {
    const sql = `
        UPDATE customers
        SET password = $1
        WHERE cust_id = $2
    `;
    const params = [newPassword, custId];
    db.query(sql, params, (err, results) => {
        callback(err, results);
    });
};
